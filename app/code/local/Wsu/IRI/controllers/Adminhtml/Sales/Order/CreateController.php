...
class Wsu_IRI_Adminhtml_Sales_Order_CreateController extends Mage_Adminhtml_Sales_Order_CreateController {
    ...
    protected function _processActionData($action = null) {
        ...
        if (!empty($couponCode)) {
            if ($this->_getQuote()->getCouponCode() !== $couponCode) {
                $this->_getSession()->addError($this->__('"%s" coupon code is not valid.', $this->_getHelper()->escapeHtml($couponCode)));
            } else {
    ...
    public function indexAction() {
        ...
        $this->_initSession();
        $this->loadLayout();
        $this->_setActiveMenu('sales/order')->renderLayout();
    }
    ...
    public function saveAction() {
        try {
            ...
            if ($payment_Data['method'] == "iri" && $customerCreditLimit < $grandTotal) {
            ...
            if ($payment_Data == "iri" && $customerCreditLimit >= $grandTotal) {
            ...
            if ($payment_Data == "iri" && $Orderstatus == "processing") {
                if ($order->canInvoice()) {
    ...
    public function configureQuoteItemsAction() {
        ...
        try {
            ...
            $configureResult->setOk(true);
            $optionCollection = Mage::getModel('sales/quote_item_option')->getCollection()->addItemFilter(array(
                $quoteItemId
            ));
